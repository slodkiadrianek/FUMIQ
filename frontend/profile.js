 ...
 async function fetchUserData() {
    try {
        ...
        const response = await fetch(`http://${base_url}/api/v1/users/${userId}`,{
        method: "GET",
        headers: {
  ...
  async function updateProfile() {
    ...
    if (userData) {
      ...
      document.getElementById('userName').textContent = `${userData.data.user.firstname} ${userData.data.user.lastname}`;
      document.getElementById('userEmail').textContent = userData.data.user.email;
      document.getElementById('joinDate').textContent = `${date.getFullYear()}-${date.getMonth()+1}-${date.getDate()} `;
      document.getElementById('testsCreated').textContent = userData.data.user.testsCreated;
      document.getElementById('testsCompleted').textContent = userData.data.user.testsCompleted;
    }
  }
